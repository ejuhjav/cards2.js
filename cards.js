...
var cards = (function() {
    ...
    Container.prototype.extend({
        ...
        render: function(options) {
            ...
            for (var i = 0; i < this.length; i++) {
                ...
                var top = $(card.el).css('top');
                var bottom = $(card.el).css('bottom');
                var left = parseInt($(card.el).css('left'));

                top = ('auto' === top ? top : parseInt(top));
                bottom = ('auto' === bottom ? bottom : parseInt(bottom));

                if (top != card.targetTop || bottom != card.targetBottom || left != card.targetLeft) {
                    var props = {
                        top: card.targetTop,
                        bottom: card.targetBottom,
                    ...
                    if (options.immediate) {
                        ...
                        // animation to/from auto to value does not work. calculate the exact top value always
                        var windowHeight = $(opt.table).height();
                        var newPosition  = ('auto' === props['top'] ? windowHeight - props['bottom'] - opt.cardSize.height : props['top']) + 'px';
                        props['top'] = newPosition;
    ...
    Deck.prototype.extend({
        calcPosition: function(options) {
            ...
            var condenseCount = 6;
            for (var i = 0; i < this.length; i++) {
                if ('auto' !== top && i > 0 && i % condenseCount == 0) {
                    top -= 1;
                    left -= 1;
                }
                this[i].targetTop = top;
                this[i].targetBottm = bottom;
